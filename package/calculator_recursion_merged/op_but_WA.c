...
int dfs(BTNode *root)
{
    ...
    {
        if (root->data == ID)
            return root->flag = dfs(root->left) | dfs(root->right) | (getval(root->lexeme) == (-1e9 + 87)) | (getval(root->lexeme) == (-1e9 - 87));
        else
            return root->flag = (root->data == ASSIGN ? 0 : dfs(root->left)) | dfs(root->right);
    }
    return 0;
}
    ...
    if (root != NULL)
    {
        ...
        case ID:
            ...
            if (lv != 0)
            {
                ...
                if (lv != 0)
                {
                    if (retval != (-1e9 + 87) && retval != (-1e9 - 87))
        ...
        case ASSIGN:
            ...
            if (root->flag == 0)
                ...
                retval = setval(root->left->lexeme, (-1e9 - 87));
        ...
        case MULDIV:
            ...
            else if (strcmp(root->lexeme, "/") == 0)
            {
                ...
                retval = lv & rv;
            if ((root->left != NULL && root->left->flag != 0) || (root->right != NULL && root->right->flag != 0))
            {
                if (root->left != NULL && root->left->flag == 0)
                {
                    root->left->reg = search_reg();
                    printf("MOV r%d %d\n", root->left->reg, root->left->val);
                }
                if (root->right != NULL && root->right->flag == 0)
                {
                    root->right->reg = search_reg();
                    printf("MOV r%d %d\n", root->right->reg, root->right->val);
                }
                if (root->left == NULL)
                {
